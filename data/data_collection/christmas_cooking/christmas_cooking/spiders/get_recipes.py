...
class GetRecipes(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        print(response.xpath(
            '//span[contains(@class, "rate-button") and contains(@class, "rate-fivestar-btn-filled")]/text()')[
                  -1].extract())
        print(response.xpath('//div[@class = "rate-info"]/text()').extract())
        print(response.xpath(
            '//span[@class = "recipe-details__cooking-time-prep"]/span[contains(@class,"mins") or contains(@class,"hrs")]/text()').extract())
        print(response.xpath(
            '//span[@class = "recipe-details__cooking-time-cook"]/span[contains(@class,"mins") or contains(@class,"hrs")]/text()').extract())
        print(response.xpath(
            '//section[contains(@class, "recipe-details__item") and  contains(@class,"recipe-details__item--skill-level")]/span[@class = "recipe-details__text"]/text()').extract())
        print(response.xpath(
            '//section[contains(@class, "recipe-details__item") and  contains(@class,"recipe-details__item--servings")]/span[@class = "recipe-details__text"]/text()').extract())
        ...
        nutrition = "".join("|".join(entry) for entry in zip(nutrition_label, nutrition_value))

        # for li in response.xpath('//ul[@class="nutrition"]/li'):
        #	nutrient = " ".join(li.xpath('string(.)').get())
        #	nutrition.append(nutrient)
        print(nutrition)
        ingredients_content = "|".join(response.xpath('//li[@class = "ingredients-list__item"]/@content').extract())
        ...
        recipe = {"Name": response.css('h1.recipe-header__title::text').extract_first(),
                  ...
                  "Prep Time": response.xpath(
                      '//span[@class = "recipe-details__cooking-time-prep"]/span[contains(@class,"mins") or contains(@class,"hrs")]/text()').extract(),
                  "Cook Time": "|".join(response.xpath(
                      '//span[@class = "recipe-details__cooking-time-cook"]/span[contains(@class,"mins") or contains(@class,"hrs")]/text()').extract()),
                  "Skill Level": response.xpath(
                      '//section[contains(@class, "recipe-details__item") and  contains(@class,"recipe-details__item--skill-level")]/span[@class = "recipe-details__text"]/text()').extract(),
                  "Servings": response.xpath(
                      '//section[contains(@class, "recipe-details__item") and  contains(@class,"recipe-details__item--servings")]/span[@class = "recipe-details__text"]/text()').extract(),
                  "Additional Info": response.xpath('//ul[@class="additional-info"]/li/text()').extract(),
                  "Nutrition": nutrition,
                  "Ingredients": "|".join(response.xpath('//li[@class = "ingredients-list__item"]/@content').extract()),
                  "Method": "|".join(response.xpath('//li[@class = "method__item"]/p/text()').extract()),
                  "DatePublished": response.xpath('//meta[@itemprop = "datePublished"]//@content').extract()}
